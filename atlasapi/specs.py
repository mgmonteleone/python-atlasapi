...
class Host(object):
    ...
    def __init__(self, data: dict):
        if type(data) != dict:
            ...
            try:
                self.created: OptionalDateTime = parser.parse(data.get("created", None))
            except (ValueError, OverflowError):
                self.created: OptionalDateTime = data.get("created", None)
            self.group_id: OptionalStr = data.get("group_id", None)
            self.hostname: OptionalStr = data.get("hostname", None)
            self.id: OptionalStr = data.get("id", None)
            ...
            self.port: OptionalInt = data.get("port", None)
            self.replica_set_name: OptionalStr = data.get("replicaSetName", None)
            self.type: ReplicaSetTypes = ReplicaSetTypes[data.get("typeName", "NO_DATA")]
